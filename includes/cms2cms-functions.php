...
class CmsPluginFunctionsDle
{
    ...
    private $jsConfig = '{
        ...
        "wp_feedback"  : "//wordpress.org/support/plugin/cms2cms-datalife-engine-to-wp-convertor/reviews",
    ...
    {
        $cms2cms_dle_access_login = get_option('cms2cms-dle-login');
        $cms2cms_dle_access_key   = get_option('cms2cms-dle-key');

        return array(
            'email'     => $cms2cms_dle_access_login,
            'accessKey' => $cms2cms_dle_access_key
        );
    }
    ...
    public function isActivated()
    {
        ...
        return ($cms2cms_dle_access_key != false);
    }
    ...
    public function getOptions()
    {
        ...
        $login = get_option('cms2cms-dle-login');
    ...
    public function saveOptions()
    {
        ...
        if ( $key && $login ) {
            ...
            add_option('cms2cms-dle-key', $key);

            delete_option('cms2cms-dle-login');
            add_option('cms2cms-dle-login', $login);

            delete_option('cms2cms-dle-depth');
            add_option('cms2cms-dle-depth', $bridge_depth);
    ...
    {
        return $this->__('Dle', 'cms2cms-dle-migration');
    }
    ...
    {
        return $this->__('WordPress', 'cms2cms-dle-migration');
    }
    ...
    {
        return sprintf(
            $this->__('CMS2CMS: Automated %s to %s Migration ', 'cms2cms-dle-migration'),
    ...
    {
        return sprintf(
            $this->__('%s to %s', 'cms2cms-dle-migration'),
            $this->getPluginSourceName(),
            $this->getPluginTargetName()
        );
    }
    ...
    public function logOut()
    {
        if (isset($_REQUEST['_wpnonce'])) {
            ...
            if ($this->verifyFormTempKey($nonce, 'cms2cms_dle_logout')
                && $_POST['cms2cms_dle_logout'] == 1
            ) {
                $this->clearOptions();
            }
        }
